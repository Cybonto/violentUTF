#!/usr/bin/env bash
# utils.sh - Common utility functions for Linux setup (Part 1)
...
}# Function to generate all secrets upfront (with credential preservation)
...
# Function to launch Streamlit in a new terminal window (Linux version)
...
EOF
    ...
    if command_exists gnome-terminal; then
        ...
        xfce4-terminal --title="ViolentUTF Streamlit Server" -e "bash $launch_script"
    elif command_exists xterm; then
        xterm -title "ViolentUTF Streamlit Server" -e bash "$launch_script" &
        ...
        echo "📝 You can manually run Streamlit with:"
