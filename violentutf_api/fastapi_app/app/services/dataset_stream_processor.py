...
class PyRITStreamProcessor:
    ...
    async def process_pyrit_dataset_stream(
        ...
        stats = DatasetImportStats(
            ...
            total_estimated=max_prompts or 0
        )
        ...
        try:
            ...
            if hasattr(dataset, 'prompts') and hasattr(dataset.prompts, '__len__'):
                ...
                if sample_size > 0:
                    ...
                    logger.debug(f"Calculated optimal chunk size: {optimal_size} (avg_prompt_size: {avg_prompt_size:.0f})")
                    return optimal_size
    ...
    async def _validate_dataset_access(self, dataset_type: str, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            supported_types = {
                "aya_redteaming", "harmbench", "adv_bench", "xstest", "pku_safe_rlhf",
                "decoding_trust_stereotypes", "many_shot_jailbreaking", "forbidden_questions",
                "seclists_bias_testing", "wmdp"
            }
