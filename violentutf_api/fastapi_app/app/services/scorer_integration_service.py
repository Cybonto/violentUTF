...
"""Scorer Integration Service module."""
...
async def _execute_true_false_scorer(scorer_config: Dict, text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("🎯 Scorer parameters: %s", scorer_config.get("parameters", {}))
    ...
    try:
        ...
        if scorer_type == "SelfAskLikertScorer":
            ...
            if "helpful" in question.lower():
                ...
                rationale = (
                    f"Helpfulness score: {helpful_count} positive vs {unhelpful_count} negative indicators "
                ...
                rationale = (
                    f"Quality score: {quality_count} positive vs {poor_count} negative indicators "
                    f"for question: {question}"
                )
                ...
                rationale = (
                    f"Length-based score: {text_length} characters, {word_count} words "
                    f"(scale: {scale_min}-{scale_max}) for question: {question}"
                )
            ...
            return {
                ...
                "score_value_description": f"Likert scale evaluation ({scale_min}-{scale_max}) "
                f"normalized to 0-1 for: {question}",
                "score_type": "float_scale",
                "score_category": "self_ask_evaluation",
                "score_rationale": f"{rationale} (Original: {score_value}, Normalized: {normalized_score:.3f})",
                "score_metadata": f'{{"question": "{question}", "scale_min": {scale_min}, '
                f'"scale_max": {scale_max}, "original_score": {score_value}, '
                f"\"scorer_name\": \"{scorer_config.get('name', 'Unknown')}\", "
...
async def _execute_substring_scorer(scorer_config: Dict, text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "score_rationale": f"Text {'contains' if is_match else 'does not contain'} "
            f"substring '{substring}' (case-insensitive)",
...
async def _execute_category_scorer(scorer_config: Dict, text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "🎯 Executing SelfAskCategoryScorer: %s",
            scorer_config.get("name", "Unknown"),
        )
        ...
        return {
            ...
            "score_value_description": f"Category: {selected_category} (index {category_index} of {len(categories)}) "
            f"for question: {question}",
            "score_type": "float_scale",  # Use float_scale for PyRIT compatibility
            "score_category": "classification",
            "score_rationale": f"Text classified as '{selected_category}' based on content analysis. "
            f"Category scores: {category_scores}",
            "score_metadata": f'{{"question": "{question}", "categories": {categories}, '
            f'"selected_category": "{selected_category}", "category_index": {category_index}, '
...
async def _execute_threshold_scorer(scorer_config: Dict, text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "🎯 Executing FloatScaleThresholdScorer: %s",
            scorer_config.get("name", "Unknown"),
        )
        ...
        return {
            ...
            "score_rationale": f"Base score {base_score:.3f} {'passes' if passes_threshold else 'fails'} "
            f"threshold {threshold}",
            "score_metadata": f'{{"threshold": {threshold}, "base_score": {base_score}, '
...
async def _execute_inverter_scorer(scorer_config: Dict, text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "🎯 Executing TrueFalseInverterScorer: %s",
            scorer_config.get("name", "Unknown"),
        )
        ...
        return {
            ...
            "score_rationale": f"Base result {base_result} inverted to {inverted_result} "
            f"(positive: {positive_count}, negative: {negative_count})",
            "score_metadata": f'{{"base_result": {base_result}, "inverted_result": {inverted_result}, '
...
async def _execute_generic_scorer(scorer_config: Dict, text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "score_metadata": f"{{\"scorer_name\": \"{scorer_config.get('name', 'Unknown')}\", "
        f"\"scorer_type\": \"{scorer_config.get('type', 'generic')}\"}}",
    }
...
async def get_scorer_by_name(scorer_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            "get_scorer_by_name called for %s - this function is deprecated",
            scorer_name,
        )
        return None
