...
class TemplateService:
    ...
    async def create_template(self, template_data: COBTemplateCreate, user_id: str) -> COBTemplateResponse:
        ...
        template_config = template_data.config.copy() if hasattr(template_data, 'config') and template_data.config else {}
        ...
        template = COBTemplate(
            ...
            tags=template_data.metadata.tags if hasattr(template_data, 'metadata') and hasattr(template_data.metadata, 'tags') else [],
    ...
    async def get_template_versions(self, template_id: str, limit: int = 10) -> List[TemplateVersionResponse]:
        ...
        stmt = select(COBTemplateVersion).where(
            COBTemplateVersion.template_id == template_id
        ).order_by(COBTemplateVersion.created_at.desc()).limit(limit)
    ...
    async def restore_template_version(self, template_id: str, version_id: str, user_id: str) -> COBTemplateResponse:
        ...
        await self._create_version(
            ...
            template.version,
            template.template_config,
            f"Restored to version {version.version}",
            user_id
        )
        ...
        for idx, block in enumerate(blocks):
            ...
            if missing:
                validation_results["warnings"].append(
                    f"Block '{block_id}' missing variables: {', '.join(missing)}"
                )
    ...
    return [
        {
            ...
            "config": {
                "blocks": [
                    {"block_type": "title", "config": {"title": "Security Assessment Report", "subtitle": "AI Model Vulnerability Analysis"}},
        ...
        },
        {
            ...
            "config": {
                "blocks": [
                    {"block_type": "title", "config": {"title": "Compliance Report", "subtitle": "Safety and Regulatory Assessment"}},
                    {"block_type": "executive_summary", "config": {}},
