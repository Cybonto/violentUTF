...
class PartialImportRecovery:
    ...
    async def attempt_recovery(self) -> Dict[str, Any]:
        ...
        if not self.config.enable_partial_import:
            raise DatasetStreamingError(
                "Partial import recovery is disabled in configuration"
            )
        ...
        retry_strategy = RetryStrategy(
            max_retries=2,  # Less aggressive retries for recovery
            base_delay=0.5,
            backoff_factor=1.5
        )
    ...
    async def _recover_single_chunk(
        ...
        if not chunk_data:
            raise DatasetStreamingError(
                f"No data available for chunk {chunk_index} recovery"
            )
        ...
        dataset_logger.debug(
            f"Successfully recovered chunk {chunk_index}",
            chunk_index=chunk_index
        )
