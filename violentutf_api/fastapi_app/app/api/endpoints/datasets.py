...
async def transform_dataset(
    ...
    try:
        ...
        db_manager = get_duckdb_manager(user_id)
        transformed_prompts_text = [p.value for p in transformed_prompts]

        # Create transformed dataset in DuckDB
        actual_transformed_id = db_manager.create_dataset(
            name=f"{original_dataset['name']}_transformed",
            source_type=DatasetSourceType.TRANSFORM.value,
            configuration={
                "original_dataset_id": dataset_id,
                "template": request.template,
                "template_type": request.template_type,
            },
            prompts=transformed_prompts_text,
        )

        # Update the ID with the actual ID from DuckDB
        transformed_dataset_data["id"] = actual_transformed_id

        transform_summary = f"Applied template to {len(original_prompts)} prompts using {request.template_type} template"

        logger.info(
            f"Dataset {dataset_id} transformed successfully, new ID: {actual_transformed_id}"
        )
