...
async def login(request: Request, form_data: OAuth2PasswordRequestForm = Depends()) -> object:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    logger.info(
                        "Keycloak token verification - username: %s, display_name: %s", username, user_info.get("name")
                    )
                    ...
                    logger.error("Unexpected error during Keycloak verification: %s", str(e))
                    log_authentication_failure(
                        username=form_data.username,
                        request=request,
                ...
                log_token_event(
                    event_type="created",
                    username=username,
                    token_type="access",  # nosec B106 - OAuth2 token type constant, not a password
                    request=request,
                )

                return Token(
                    access_token=access_token,
                    token_type="bearer",  # nosec B106 - OAuth2 token type constant, not a password
        ...
        logger.error("Keycloak connection error: %s", str(e))
        log_authentication_failure(
            username=form_data.username,
            request=request,
        ...
        raise authentication_error("Authentication failed") from e
...
async def check_password_strength(request: Request) -> object:
    ...
    try:
        ...
        logger.error("Password strength check error: %s", str(e))
        raise safe_error_response("Failed to check password strength", status_code=500) from e
