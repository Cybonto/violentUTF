...
async def login(request: Request, form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    log_authentication_failure(
                        username=form_data.username, request=request, reason="Keycloak token verification error"
                    )
                    raise HTTPException(
                ...
                log_authentication_success(username=username, user_id=username, request=request, keycloak_auth=True)

                # Log token creation
                log_token_event(event_type="created", username=username, token_type="access", request=request)
        ...
        log_authentication_failure(
            username=form_data.username, request=request, reason="Authentication service unavailable"
        )
        raise safe_error_response(
            "Authentication service temporarily unavailable", status_code=status.HTTP_503_SERVICE_UNAVAILABLE
        )
    except Exception as e:
        # Catch any other authentication errors
        logger.error(f"Unexpected authentication error: {str(e)}")
        log_authentication_failure(
            username=form_data.username, request=request, reason="Unexpected authentication error"
        )
...
async def validate_token(
    ...
    try:
        ...
        return TokenValidationResponse(
            valid=False, username=None, roles=[], has_ai_access=False, missing_roles=[], error=str(e)
        )
