...
"""MCP resource manager for handling resource lifecycle and coordination.
...
class ViolentUTFResourceManager:
    """Manage ViolentUTF resources for MCP access."""

    def __init__(self: ViolentUTFResourceManager) -> None:
        """Initialize instance."""
    ...
    async def list_resources(self: ViolentUTFResourceManager) -> List[Resource]:
        ...
        try:
            ...
            logger.info(
                "Listed %s MCP resources (%s advanced, %s legacy)",
                len(mcp_resources),
                len(advanced_resources),
                len(legacy_resources),
            )
    ...
    async def _list_generator_resources(
        self: ViolentUTFResourceManager,
    ) -> List[Resource]:
    ...
    async def _list_orchestrator_resources(
        self: ViolentUTFResourceManager,
    ) -> List[Resource]:
    ...
    async def _read_generator_resource(self: ViolentUTFResourceManager, generator_id: str) -> Optional[Dict[str, Any]]:
        """Read generator configuration details."""
        return await self._api_request("GET", f"/api/v1/generators/{generator_id}")

    async def _read_dataset_resource(self: ViolentUTFResourceManager, dataset_name: str) -> Optional[Dict[str, Any]]:
        """Read dataset details."""
        return await self._api_request("GET", f"/api/v1/datasets/{dataset_name}")

    async def _read_orchestrator_resource(
        self: ViolentUTFResourceManager, orchestrator_id: str
    ...
    async def _api_request(
        self: ViolentUTFResourceManager, method: str, path: str, **kwargs: object
    ) -> Optional[Dict[str, Any]]:
    ...
    def get_cache_stats(self: ViolentUTFResourceManager) -> Dict[str, Any]:
        ...
        for _, cached_data in self.resource_cache.items():
