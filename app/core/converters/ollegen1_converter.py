...
class OllaGen1DatasetConverter:
    ...
    def batch_convert_rows(self, rows: List[Dict[str, str]]) -> List[QuestionAnsweringEntry]:
        ...
        for row in rows:
            try:
                ...
                successful_count = self._conversion_stats.get("successful_conversions") or 0
                self._conversion_stats["successful_conversions"] = successful_count + 1
                total_qa_count = self._conversion_stats.get("total_qa_entries") or 0
                self._conversion_stats["total_qa_entries"] = total_qa_count + len(qa_entries)
            except Exception as e:
                failed_count = self._conversion_stats.get("failed_conversions") or 0
                self._conversion_stats["failed_conversions"] = failed_count + 1
                logging.warning("Failed to convert row %s: %s", row.get("ID", "unknown"), e)
                continue

            total_processed = self._conversion_stats.get("total_processed") or 0
    ...
    def batch_convert_with_recovery(self, rows: List[Dict[str, str]]) -> OllaGen1BatchConversionResult:
        ...
        """
        start_time = time.time()
        self._conversion_stats["start_time"] = int(start_time)
    ...
    async def convert_file(self, file_path: str) -> Dict[str, Any]:
        ...
        """
        try:
            # Read the file first to get rows
            if not os.path.exists(file_path):
