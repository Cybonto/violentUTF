...
async def test_generator_integration_apisix():
    ...
    with patch('violentutf_api.fastapi_app.app.services.generator_integration_service.get_generator_by_name') as mock_get_gen:
        ...
        with patch('requests.post') as mock_post:
            ...
            result = await execute_generator_prompt(generator_name, prompt, conversation_id)
...
async def test_dataset_integration_native():
    ...
    with patch('violentutf_api.fastapi_app.app.services.dataset_integration_service._get_dataset_by_id') as mock_get_dataset:
...
async def test_scorer_integration_true_false():
    ...
    with patch('violentutf_api.fastapi_app.app.services.scorer_integration_service.get_scorer_by_name') as mock_get_scorer:
        mock_get_scorer.return_value = mock_scorer_config
        
        result = await execute_scorer(scorer_name, text)
        
        assert result["score_value"] == 1.0  # Should be 1.0 because text contains "helpful"
...
async def test_end_to_end_orchestrator_workflow():
    ...
    with patch('violentutf_api.fastapi_app.app.services.generator_integration_service.get_generator_by_name') as mock_get_gen:
        ...
        with patch('violentutf_api.fastapi_app.app.services.generator_integration_service.execute_generator_prompt') as mock_exec_gen:
...
async def test_orchestrator_with_dataset_execution():
    ...
    with patch('violentutf_api.fastapi_app.app.services.generator_integration_service.get_generator_by_name') as mock_get_gen:
        ...
        with patch('violentutf_api.fastapi_app.app.services.generator_integration_service.execute_generator_prompt') as mock_exec_gen:
            ...
            with patch('violentutf_api.fastapi_app.app.services.dataset_integration_service.get_dataset_prompts') as mock_get_dataset:
