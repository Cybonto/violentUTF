...
class TestEnhancementStripUI:
    ...
    def test_enhancement_strip_appears_with_input(self, mock_session_state):
        ...
        mock_session_state["mcp_parser"].parse.return_value = Mock(
    ...
    def test_enhance_button_functionality(self, mock_session_state):
        ...
        enhanced_prompt = (
            "Enhanced: Test prompt with improved clarity and effectiveness"
        )
        ...
        # Call enhance
        result = mock_session_state["mcp_client"].get_prompt(
            "prompt_enhancement", {"original_prompt": user_input}
        )
    ...
    def test_analyze_button_functionality(self, mock_session_state):
        ...
        analysis = (
            "Analysis: No security issues detected. Bias score: Low. Quality: Good."
        )
        mock_session_state["mcp_client"].get_prompt.return_value = analysis

        # Simulate analyze button click
        result = mock_session_state["mcp_client"].get_prompt(
            "security_analysis", {"prompt": user_input}
        )
    ...
    def test_test_button_functionality(self, mock_session_state):
        ...
        for test_type in ["jailbreak_test", "bias_detection", "privacy_test"]:
            variation = mock_session_state["mcp_client"].get_prompt(
                test_type, {"context": user_input}
            )
            if variation:
    ...
    def test_results_display(self, mock_session_state):
        ...
        mock_session_state["mcp_test_variations"] = [
            {"type": "test1", "prompt": "Test variation 1"}
        ]
    ...
    def test_error_handling(self, mock_session_state):
        ...
        mock_session_state["mcp_client"].get_prompt.side_effect = Exception(
            "Test error"
        )
        ...
        try:
            result = mock_session_state["mcp_client"].get_prompt(
                "prompt_enhancement", {"original_prompt": "test"}
            )
...
class TestSessionStateIntegration:
    ...
    def test_prompt_variable_integration(self):
        ...
        resolved = enhanced_prompt.replace(
            "{{greeting}}", prompt_variables["greeting"]["value"]
        )
...
class TestUserFlows:
    ...
    def test_enhance_and_use_flow(self):
        ...
        # Enhance operation
        result = session_state["mcp_client"].get_prompt(
            "prompt_enhancement", {"original_prompt": user_input}
        )
