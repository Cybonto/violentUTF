...
class TestMCPClient:
    ...
    async def test_read_resource(self, client, mock_auth_headers):
        ...
        mock_response.json.return_value = {
            ...
            "result": {
                "contents": [
                    {"mimeType": "text/plain", "text": "Resource content here"}
                ]
            },
        }
...
"""
    ...
    async def test_auth_header_generation(self, client):
        ...
        with patch(
            "utils.jwt_manager.jwt_manager.get_valid_token", return_value="test_token"
        ):
...
class TestMCPClientSync:
    ...
    def test_sync_initialization(self, client):
        ...
        with patch.object(
            client.client, "initialize", new_callable=AsyncMock, return_value=True
        ):
    ...
    async def test_full_workflow(self):
        ...
        responses = [
            ...
            # Get prompt response
            {
                ...
                "result": {
                    "messages": [
                        {"role": "user", "content": "Testing security for: example.com"}
                    ]
                },
            },
        ]
        ...
        with patch(
            ...
            with patch(
                ...
                prompt_text = await client.get_prompt(
                    "security_test", {"target": "example.com"}
                )
