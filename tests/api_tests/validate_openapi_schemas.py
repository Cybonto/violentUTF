                ...
                if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                    ...
                    for code in error_codes:
                        if code in responses:
                            response_schema = responses[code].get("content", {}).get("application/json", {}).get("schema", {})
                            
                            # Validate error response structure
                            if not self._is_valid_error_schema(response_schema):
                                self.errors.append(f"Invalid error response schema for {method.upper()} {path} {code} in {source}")
                                return False
