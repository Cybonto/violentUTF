...
class TestConverterApply:
    ...
    def create_test_converter(self, name: str, converter_type: str) -> str:
        ...
        response = requests.post(
            API_ENDPOINTS["converters"], json=payload, headers=self.headers
        )
    ...
    def test_converter_apply_copy_mode(self):
        ...
        response = requests.get(
            f"{API_ENDPOINTS['datasets']}/{new_dataset_id}", headers=self.headers
        )
    ...
    def test_converter_apply_missing_new_name(self):
        ...
        converter_id = self.create_test_converter(
            "validation_converter", "ROT13Converter"
        )
