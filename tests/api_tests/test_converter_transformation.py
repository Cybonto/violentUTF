    ...
    try:
        ...
        dataset_payload = {
            ...
            "file_content": base64.b64encode(
                json.dumps(test_prompts).encode()
            ).decode(),
            "file_type": "json",
            "config": {},
        }
        ...
        response = requests.post(
            f"{API_BASE_URL}/api/v1/datasets", json=dataset_payload, headers=headers
        )
        ...
        converter_payload = {
            ...
            "parameters": {
                "append_description": False  # Don't append description for easier verification
            },
        }

        response = requests.post(
            f"{API_BASE_URL}/api/v1/converters", json=converter_payload, headers=headers
        )
        ...
        response = requests.get(
            f"{API_BASE_URL}/api/v1/datasets/{new_dataset_id}", headers=headers
        )
        ...
        for converter_id in resources_to_cleanup["converters"]:
            try:
                requests.delete(
                    f"{API_BASE_URL}/api/v1/converters/{converter_id}", headers=headers
                )
            except:
                pass
        for dataset_id in resources_to_cleanup["datasets"]:
            try:
                requests.delete(
                    f"{API_BASE_URL}/api/v1/datasets/{dataset_id}", headers=headers
                )
            except:
                pass
