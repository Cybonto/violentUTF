...
class TestEnhancedGeneratorFunctionality:
    ...
    def test_get_generator_types_live(self, api_headers, api_base_url):
        ...
        response = requests.get(f"{api_base_url}/api/v1/generators/types", 
    ...
    def test_get_ai_gateway_parameters_live(self, api_headers, api_base_url):
        ...
        response = requests.get(f"{api_base_url}/api/v1/generators/types/AI Gateway/params", 
                              headers=api_headers, timeout=10)
    ...
    def test_get_apisix_models_live(self, api_headers, api_base_url):
        ...
        for provider in providers:
            ...
            assert response.status_code == 200, f"Failed to get models for {provider}: {response.status_code} - {response.text}"
    ...
    def test_create_anthropic_generator_live(self, api_headers, api_base_url, cleanup_generators):
        ...
        assert response.status_code == 200, f"Failed to create Anthropic generator: {response.status_code} - {response.text}"
    ...
    def test_generator_testing_live(self, api_headers, api_base_url, cleanup_generators):
        ...
        if create_response.status_code == 401:
            pytest.fail(f"Authentication failed during generator creation: {create_response.text}")
        ...
        assert "missing api key in request" not in response_text, f"APISIX API key issue: {test_response.text}"
        ...
        if test_response.status_code == 200:
            ...
            if is_acceptable:
                print(f"⚠️  Generator test failed due to AI provider issues (acceptable): {test_response.status_code}")
                pytest.skip(f"AI provider issue: {test_response.text}")
            else:
                pytest.fail(f"Generator test failed with unexpected error: {test_response.status_code} - {test_response.text}")
    ...
    def test_list_generators_live(self, api_headers, api_base_url, cleanup_generators):
        ...
        # Get initial count
        list_response = requests.get(f"{api_base_url}/api/v1/generators", 
                                   headers=api_headers, timeout=10)
        
        if list_response.status_code == 401:
            pytest.fail(f"Authentication failed: {list_response.text}")
            
        assert list_response.status_code == 200, f"Failed to list generators: {list_response.status_code}"
        ...
        assert generator_config["name"] in generator_names, "Created generator not found in list"
