...
class TestConverterApplyFunctionality:
    ...
    def cleanup_resources(self):
        ...
        for dataset_id in self.created_resources["datasets"]:
            try:
                requests.delete(
                    f"{API_BASE}/datasets/{dataset_id}", headers=self.headers
                )
            except:
                pass
        ...
        for converter_id in self.created_resources["converters"]:
            try:
                requests.delete(
                    f"{API_BASE}/converters/{converter_id}", headers=self.headers
                )
            except:
                pass
    ...
    def test_converter_apply_copy_mode(self):
        ...
        response = requests.post(
            f"{API_BASE}/datasets", json=dataset_data, headers=self.headers
        )
        ...
        response = requests.post(
            f"{API_BASE}/converters", json=converter_data, headers=self.headers
        )
        assert (
        ...
        response = requests.post(
            f"{API_BASE}/converters/{converter_id}/apply",
            json=apply_data,
        ...
        assert (
            response.status_code == 200
        ), f"Failed to apply converter: {response.text}"
        ...
        logger.info(
            f"Successfully applied converter in COPY mode, created dataset: {result['dataset_id']}"
        )
    ...
    def test_converter_apply_overwrite_mode(self):
        ...
        logger.info(
            f"Successfully applied converter in OVERWRITE mode, dataset ID: {new_dataset_id}"
        )
    ...
    def test_converter_apply_with_parameters(self):
        ...
        response = requests.post(
            f"{API_BASE}/converters/{converter_id}/preview",
            json=preview_data,
            headers=self.headers,
        )
        assert (
            response.status_code == 200
        ), f"Failed to preview converter: {response.text}"
    ...
    def test_converter_apply_missing_new_name_for_copy(self):
        ...
        assert "new_dataset_name is required" in error_data.get(
