...
class TestDatasetPromptFormat:
    ...
    def cleanup_resources(self):
        ...
        for dataset_id in self.created_resources["datasets"]:
            try:
                requests.delete(
                    f"{API_ENDPOINTS['datasets']}/{dataset_id}", headers=self.headers
                )
            except:
                pass
    ...
    def test_dataset_creation_and_retrieval(self):
        ...
        response = requests.post(
            API_ENDPOINTS["datasets"], json=payload, headers=self.headers
        )
        ...
        ), f"Failed to retrieve dataset: {response.text}"
        ...
        if retrieved_dataset["prompts"]:
            ...
            if "text" in first_prompt:
                ...
                print(
                    f"⚠️  Prompts use 'prompt_text' field, but converters expect 'text'"
                )
            else:
                print(
                    f"❌ Prompts don't have expected field. Keys: {list(first_prompt.keys())}"
                )
    ...
    def test_converter_apply_with_dataset(self):
        ...
        response = requests.post(
            API_ENDPOINTS["converters"], json=converter_payload, headers=self.headers
        )
...
def main():
    ...
    try:
        ...
        print("\n📝 Key Findings:")
        print(
            "- Dataset prompts have inconsistent field naming across different layers"
        )
        print(
            "- The DuckDB manager correctly maps 'prompt_text' to 'text' for API consistency"
        )
        print(
            "- Converters expect the 'text' field which matches the DuckDB manager output"
        )
