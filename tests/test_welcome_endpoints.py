...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "..", "violentutf_api", "fastapi_app")
)
...
class TestAuthenticationEndpoints:
    ...
    def test_validate_token(self, mock_get_user, auth_headers, mock_user):
        ...
        response = client.post(
            "/api/v1/auth/token/validate", headers=auth_headers, json=payload
        )
        assert response.status_code == 200
    ...
    def test_validate_token_missing_role(self, mock_get_user, auth_headers):
        ...
        mock_user_no_access = type(
            "MockUser",
            (),
            {"username": "testuser", "email": "test@example.com", "roles": []},
        )()
...
class TestDatabaseEndpoints:
    ...
    def test_initialize_database(
        ...
        response = client.post(
            "/api/v1/database/initialize", headers=auth_headers, json=payload
        )
        assert response.status_code == 200
    ...
    def test_reset_database(self, mock_exists, mock_get_user, auth_headers, mock_user):
        ...
        response = client.post(
            "/api/v1/database/reset", headers=auth_headers, json=payload
        )
        assert response.status_code == 200
...
class TestSessionEndpoints:
    ...
    def test_get_session_state(
        self, mock_exists, mock_file, mock_get_user, auth_headers, mock_user
    ):
    ...
    def test_reset_session_state(
        self, mock_makedirs, mock_file, mock_get_user, auth_headers, mock_user
    ):
...
class TestConfigEndpoints:
    ...
    def test_update_config_parameters(
        ...
        payload = {
            "parameters": {"APP_DATA_DIR": "./new_data_dir", "new_setting": "value"},
            "merge_strategy": "merge",
        }

        response = client.put(
            "/api/v1/config/parameters", headers=auth_headers, json=payload
        )
        assert response.status_code == 200
...
"""
        ...
        with patch("builtins.open", mock_open()) as mock_file:
            response = client.post(
                "/api/v1/config/parameters/load", headers=auth_headers, files=files
            )
            assert response.status_code == 200
    ...
    def test_list_parameter_files(
        self, mock_glob, mock_exists, mock_get_user, auth_headers, mock_user
    ):
        ...
        mock_files = [
            type(
                ...
                (),
                {
                    ...
                    "stat": lambda: type(
                        "MockStat", (), {"st_size": 1024, "st_mtime": 1700654921}
                    )(),
                },
            )()
        ]
...
class TestEnvironmentConfigEndpoints:
    ...
    def test_update_environment_config(self, mock_get_user, auth_headers, mock_user):
        ...
        response = client.put(
            "/api/v1/config/environment", headers=auth_headers, json=payload
        )
        assert response.status_code == 200
    ...
    def test_validate_environment_config(self, mock_get_user, auth_headers, mock_user):
        ...
        with patch.dict(os.environ, {"PYRIT_DB_SALT": "short"}):
            response = client.post(
                "/api/v1/config/environment/validate", headers=auth_headers
            )
            assert response.status_code == 200
    ...
    def test_generate_salt(self, mock_get_user, auth_headers, mock_user):
        ...
        response = client.post(
            "/api/v1/config/environment/generate-salt", headers=auth_headers
        )
        assert response.status_code == 200
...
class TestFileEndpoints:
    ...
    def test_upload_file(
        ...
        response = client.post(
            "/api/v1/files/upload", headers=auth_headers, files=files
        )
        assert response.status_code == 200
