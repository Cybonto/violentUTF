...
def test_orchestrator_types():
    ...
    try:
        response = requests.get("http://localhost:9080/api/v1/orchestrators/types", headers=headers, timeout=10)
...
def test_dataset_endpoints():
    ...
    try:
        response = requests.get("http://localhost:9080/api/v1/datasets", headers=headers, timeout=10)
        ...
        if response.status_code == 200:
            ...
            for dataset in datasets:
                print(f"  - {dataset.get('name', 'Unknown')}: {dataset.get('prompt_count', 0)} prompts")
            return True
        else:
            print(f"❌ Failed: {response.status_code} - {response.text}")
            return False
...
def test_generator_endpoints():
    ...
    try:
        response = requests.get("http://localhost:9080/api/v1/generators", headers=headers, timeout=10)
        ...
        if response.status_code == 200:
            ...
            for generator in generators:
                print(f"  - {generator.get('name', 'Unknown')}: {generator.get('type', 'Unknown')}")
            return True
        else:
            print(f"❌ Failed: {response.status_code} - {response.text}")
            return False
