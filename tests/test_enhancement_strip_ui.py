...
class TestEnhancementStripUI:
    ...
    def test_quick_actions_dropdown_exists(self):
        ...
        selectbox_calls = []
        mock_selectbox = Mock(side_effect=lambda label, **kwargs: (
            selectbox_calls.append(kwargs.get('options', [])) or "Select an action..."
        ))
        
        # Simulate the quick actions dropdown creation
        expected_actions = ["Select an action...", "Test for jailbreak", "Check for bias", "Privacy analysis", "Security audit"]
