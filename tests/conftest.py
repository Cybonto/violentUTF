...
def print_test_environment(setup_test_environment):
    ...
    print("=" * 60)
    print(
        f"🔌 APISIX Gateway:        {'✅ Running' if env['apisix_running'] else '❌ Not Running'}"
    )
    print(
        f"⚡ FastAPI Service:       {'✅ Running' if env['fastapi_running'] else '❌ Not Running'}"
    )
    print(
        f"🔐 Keycloak Available:    {'✅ Yes' if env['keycloak_available'] else '❌ No'}"
    )
    print(
        f"🔑 JWT Secret:            {'✅ Configured' if env['jwt_secret_configured'] else '❌ Missing'}"
    )
    print(
        f"🗝️  APISIX API Key:        {'✅ Configured' if env['apisix_api_key_configured'] else '❌ Missing'}"
    )
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "requires_apisix: mark test as requiring APISIX gateway"
    )
    config.addinivalue_line(
        "markers", "requires_fastapi: mark test as requiring FastAPI service"
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    try:
        apisix_running = requests.get(
            "http://localhost:9080/health", timeout=2
    ...
    try:
        fastapi_running = (
            requests.get("http://localhost:8000/health", timeout=2).status_code == 200
        )
    except:
