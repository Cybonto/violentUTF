...
def generate_hmac_signature(
    gateway_secret: str, method: str, path: str, timestamp: str = None
) -> Tuple[str, str]:
...
def test_authentication_bypass_blocked():
    ...
    signature, _ = generate_hmac_signature(
        gateway_secret, "GET", "/health", old_timestamp
    )
    ...
    valid_signature, _ = generate_hmac_signature(
        gateway_secret, "GET", "/health", current_timestamp
    )
...
def test_timing_attack_resistance():
    ...
    secret = "test_secret"
    correct_sig, timestamp = generate_hmac_signature(
        secret, "GET", "/test", "1609459200"
    )
