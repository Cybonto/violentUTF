...
class TestMCPIntegration:
    ...
    async def test_list_real_tools(self, async_client):
        ...
        if len(tools) == 0:
            ...
            logger.warning(
                "No tools returned - possible JSON serialization issue in MCP server"
            )
        ...
        assert any(
            "generator" in name for name in tool_names
        ), "Should have generator-related tools"
        assert any(
            "dataset" in name for name in tool_names
        ), "Should have dataset-related tools"
    ...
    def test_performance_baseline(self, sync_client):
        ...
        ), f"Health check took too long: {health_check_time}s"
