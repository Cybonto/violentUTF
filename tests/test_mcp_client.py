...
from violentutf.utils.mcp_client import (
    MCPClient, MCPClientSync, MCPResponse, MCPMethod,
    MCPClientError, MCPConnectionError, MCPAuthenticationError, MCPTimeoutError
)
...
class TestMCPClient:
    ...
    async def test_send_request_success(self, client):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            mock_response.iter_lines = AsyncMock(return_value=[
    ...
    async def test_execute_tool(self, client):
        ...
        mock_response = MCPResponse(
            id=1,
            result={"success": True, "generator_id": "gen-123"}
        )
    ...
    async def test_get_prompt(self, client):
        ...
        mock_response = MCPResponse(
            ...
            result={
                "messages": [
                    {"role": "user", "content": "Enhanced prompt text"}
                ]
            }
        )
    ...
    async def test_read_resource(self, client):
        ...
        mock_response = MCPResponse(
            id=1,
            result={"content": {"prompts": ["test1", "test2"]}}
        )
