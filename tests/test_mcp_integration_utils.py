...
class TestNaturalLanguageParser:
    ...
    def test_extract_parameters(self, parser):
        ...
        params = parser.extract_parameters(
            "create a generator with temperature 0.8 and max_tokens 1000"
        )
...
class TestConfigurationIntentDetector:
    ...
    def test_extract_generator_params(self, detector):
        ...
        params = detector.extract_generator_params(
        ...
        params = detector.extract_generator_params(
            "OpenAI generator using gpt-3.5-turbo with max_tokens 2000"
        )
...
class TestContextAnalyzer:
    ...
    def test_analyze_security_context(self, analyzer):
        ...
        assert any(
            "dataset" in action.lower() for action in analysis["suggested_actions"]
        )
    ...
    def test_detect_incomplete_workflow(self, analyzer):
        ...
        assert any(
            "test" in s.lower() or "orchestrator" in s.lower() for s in suggestions
        )
