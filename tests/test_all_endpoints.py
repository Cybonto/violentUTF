...
def test_endpoint(
    ...
    #     global test_results  # TODO: Remove or use this global
    ...
    success, status_code, message, response_data = make_request(
...
def test_generator_endpoints():
    ...
    gen_created = test_endpoint(
        "POST", "/api/v1/generators", "Create generator", generator_payload
    )
...
def test_dataset_endpoints():
    ...
    dataset_created = test_endpoint(
        "POST", "/api/v1/datasets", "Create dataset", dataset_payload
    )
...
def test_converter_endpoints():
    ...
    test_endpoint(
        "GET", "/api/v1/converters/params/ROT13Converter", "Get converter params"
    )
    ...
    converter_created = test_endpoint(
        "POST", "/api/v1/converters", "Create converter", converter_payload
    )
...
def test_scorer_endpoints():
    ...
    test_endpoint(
        "POST", "/api/v1/scorers/validate", "Validate scorer config", validation_payload
    )
    ...
    scorer_created = test_endpoint(
        "POST", "/api/v1/scorers", "Create scorer", scorer_payload
    )
...
def print_summary():
    ...
    success_rate = (
        (test_results["passed"] / (test_results["total"] - test_results["skipped"]))
        * 100
