...
sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / "violentutf_api" / "fastapi_app"))
...
class TestGarakIntegrationService:
    ...
    def test_initialization_without_garak(self):
        ...
        with patch('app.services.garak_integration.GarakService._initialize_garak', 
    ...
    def test_get_available_probes(self, garak_service):
        ...
        mock_garak._plugins.enumerate_plugins = Mock(return_value=["encoding", "malwaregen"])
        
        # Mock probe modules
        mock_encoding_module = Mock()
        mock_encoding_module.InjectAscii85 = Mock()
        mock_encoding_module.InjectAscii85.description = "Inject ASCII85 encoded payloads"
        mock_encoding_module.InjectAscii85.tags = ["encoding", "injection"]
        mock_encoding_module.InjectAscii85.goal = "Test ASCII85 encoding vulnerabilities"
    ...
    def test_get_available_probes_error(self, garak_service):
        ...
        mock_garak._plugins.enumerate_plugins = Mock(side_effect=Exception("Plugin error"))
    ...
    def test_get_available_detectors(self, garak_service):
        ...
        # Mock the enumerate_plugins function
        mock_garak._plugins.enumerate_plugins = Mock(return_value=["encoding", "malware"])
        
        # Mock detector modules
        mock_encoding_module = Mock()
        mock_encoding_module.DecodeAscii85 = Mock()
        mock_encoding_module.DecodeAscii85.description = "Detect ASCII85 encoded content"
    ...
    @pytest.mark.asyncio
    async def test_run_scan_success(self, garak_service, scan_config, mock_probe, mock_detector):
            ...
            "results": [
                {
                    ...
                    "detector_results": [
                        {"detector": "encoding.DecodeAscii85", "passed": True, "score": 0.95}
                    ]
                }
            ],
            "summary": {
        ...
        with patch.object(garak_service, 'create_scan_harness', return_value=mock_harness):
    ...
    async def test_run_scan_with_error(self, garak_service, scan_config):
        ...
        with patch.object(garak_service, 'create_scan_harness', 
    ...
    def test_create_scan_harness(self, garak_service, scan_config):
        ...
        with patch.dict('sys.modules', {'garak.harnesses.probewise': Mock(Harness=mock_harness_class)}):
            with patch.object(garak_service, 'create_scan_harness', return_value=mock_harness_instance):
        ...
        with patch.object(garak_service, 'run_scan', side_effect=mock_long_scan):
            with pytest.raises(asyncio.TimeoutError):
                await asyncio.wait_for(
                    garak_service.run_scan(scan_config),
                    timeout=1.0
                )
    ...
    def test_concurrent_scans(self, garak_service):
        ...
        configs = [
            {"scan_id": f"scan{i}", "probes": [f"probe{i}"]} 
            for i in range(3)
        ]
    ...
    def test_malicious_probe_name(self, garak_service):
        ...
        for name in malicious_names:
            ...
            with patch.object(mock_garak._plugins, 'load_plugin', return_value=None):
