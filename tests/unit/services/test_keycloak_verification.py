...
class TestKeycloakJWTVerifier:
    ...
    def mock_jwks_response(self):
        ...
        def to_base64url(num):
            ...
            return (
                base64.urlsafe_b64encode(num.to_bytes(byte_length, "big"))
                .decode("ascii")
                .rstrip("=")
            )
    ...
    @pytest.mark.asyncio
    async def test_get_keycloak_config_cache_expired(
        self, verifier, mock_keycloak_config
    ):
    ...
    async def test_verify_token_success(
        self, verifier, mock_jwt_token, mock_keycloak_config, mock_jwks_response
    ):
        ...
        with patch.object(
            verifier, "_get_keycloak_config", return_value=mock_keycloak_config
        ):
            with patch("jwt.PyJWKClient") as mock_jwks_client_class:
                mock_jwks_client = Mock()
                mock_signing_key = Mock()
                mock_signing_key.key = jwks_data["keys"][0]
                mock_jwks_client.get_signing_key_from_jwt.return_value = (
                    mock_signing_key
                )
    ...
    async def test_verify_token_wrong_issuer(
        self, verifier, mock_keycloak_config, mock_jwt_token, mock_jwks_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_verify_token_none_algorithm_attack(
        self, verifier, mock_keycloak_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_verify_token_key_confusion_attack(
        self, verifier, mock_keycloak_config
    ):
        """Test protection against key confusion attacks"""
        # Token signed with symmetric key instead of RSA
        fake_token = jwt.encode(
            {"sub": "admin", "iss": verifier.issuer}, "secret-key", algorithm="HS256"
        )
        ...
        with patch.object(
            ...
            with patch("jwt.PyJWKClient"):
                with patch(
                    "jwt.decode",
                    side_effect=jwt.InvalidAlgorithmError("Invalid algorithm"),
                ):
                    with pytest.raises(HTTPException) as exc_info:
