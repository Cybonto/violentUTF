...
class TestPyRITOrchestratorService:
    ...
    def orchestrator_service(self):
        ...
        with patch('app.services.pyrit_orchestrator_service.CentralMemory') as mock_memory:
    ...
    def test_validate_memory_access_error(self, orchestrator_service):
        ...
        with patch.object(orchestrator_service, '_get_memory', side_effect=Exception("Memory error")):
            result = orchestrator_service.validate_memory_access()
    ...
    def test_create_target_import_error(self, orchestrator_service):
        ...
        with patch('app.services.pyrit_orchestrator_service.import_module', side_effect=ImportError("Module not found")):
    ...
    def test_create_orchestrator_prompt_sending(self, mock_memory_class, orchestrator_service):
        ...
        with patch('app.services.pyrit_orchestrator_service.PromptSendingOrchestrator') as mock_orch_class:
            ...
            assert orchestrator_service._orchestrator_instances[orch_id] == mock_orch_instance
    ...
    @pytest.mark.asyncio
    async def test_run_orchestrator_success(self, orchestrator_service, mock_orchestrator):
        ...
        assert orchestrator_service._orchestrator_metadata["test-id"]["status"] == "completed"
    ...
    @pytest.mark.asyncio
    async def test_run_orchestrator_with_seed_prompts(self, orchestrator_service, mock_orchestrator):
    ...
    @pytest.mark.asyncio
    async def test_run_orchestrator_error_handling(self, orchestrator_service, mock_orchestrator):
        ...
        assert orchestrator_service._orchestrator_metadata["test-id"]["status"] == "error"
    
    # ======================
    # Results and Memory Tests
    # ======================
    
    def test_get_orchestrator_results_success(self, orchestrator_service, mock_orchestrator):
    ...
    def test_cleanup_orchestrator_error_handling(self, orchestrator_service, mock_orchestrator):
    ...
    async def test_full_orchestrator_lifecycle(self, orchestrator_service):
        ...
        with patch('app.services.pyrit_orchestrator_service.PromptSendingOrchestrator') as mock_orch_class:
            with patch('app.services.pyrit_orchestrator_service.DuckDBMemory'):
                ...
                assert orchestrator_service._orchestrator_instances[orch_id] == mock_orch
                ...
                result = await orchestrator_service.run_orchestrator(orch_id, ["Test prompt"])
                assert result["status"] == "completed"
...
class TestPyRITServiceEdgeCases:
    ...
    async def test_concurrent_orchestrator_creation(self, orchestrator_service):
        ...
        with patch('app.services.pyrit_orchestrator_service.PromptSendingOrchestrator') as mock_orch_class:
            with patch('app.services.pyrit_orchestrator_service.DuckDBMemory'):
                ...
                mock_orch_class.side_effect = [
                    Mock(id=f"orch-{i}") for i in range(5)
                ]
                ...
                results = await asyncio.gather(*[asyncio.create_task(t) if asyncio.iscoroutine(t) else asyncio.create_task(asyncio.coroutine(lambda: t)()) for t in tasks])
    ...
    def test_memory_label_handling(self, orchestrator_service):
        ...
        with patch('app.services.pyrit_orchestrator_service.DuckDBMemory') as mock_memory_class:
            mock_memory_instance = Mock()
            mock_memory_class.return_value = mock_memory_instance
