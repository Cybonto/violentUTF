...
from app.mcp.resources.documentation import DocumentationResourceProvider, DocumentIndex, DocumentMetadata
...
class TestDocumentIndex:
    ...
    def test_search_with_category_filter(self, index_with_docs):
        ...
        assert len(results) > 0
        assert all(index_with_docs.documents[doc_id]["metadata"].category == "api" for doc_id, score in results)
...
class TestDocumentationResourceProvider:
    ...
    def test_find_docs_root_existing(self, temp_docs_dir):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch("pathlib.Path.is_dir", return_value=True), patch(
            "pathlib.Path", return_value=temp_docs_dir
        ):
            provider = DocumentationResourceProvider()
    ...
    async def test_get_resource_search_query(self, provider):
        ...
        # Valid search query
        result = await provider.get_resource("violentutf://docs/search/query", {"query": "authentication", "limit": 3})
    ...
    async def test_get_resource_search_no_query(self, provider):
        ...
        result = await provider.get_resource("violentutf://docs/search/query", {})
    ...
    async def test_get_resource_category_overview(self, provider):
        ...
        result = await provider.get_resource("violentutf://docs/api/overview", {})
    ...
    async def test_get_resource_specific_document(self, provider):
        ...
        if provider.index.documents:
            ...
            result = await provider.get_resource(f"violentutf://docs/{category}/{doc_id}", {})
    ...
    async def test_get_resource_document_with_summary(self, provider):
        ...
        if provider.index.documents:
            ...
            result = await provider.get_resource(f"violentutf://docs/{category}/{doc_id}", {"summary": True})
    ...
    async def test_get_resource_nonexistent_document(self, provider):
        ...
        result = await provider.get_resource("violentutf://docs/api/nonexistent_doc", {})
...
class TestIntegration:
    ...
    async def test_end_to_end_search(self, temp_docs_dir):
        ...
        with patch.object(DocumentationResourceProvider, "_find_docs_root", return_value=temp_docs_dir):
            ...
            result = await provider.get_resource(
                "violentutf://docs/search/query", {"query": "setup Docker", "limit": 2}
            )
    ...
    async def test_concurrent_access(self, temp_docs_dir):
        ...
        with patch.object(DocumentationResourceProvider, "_find_docs_root", return_value=temp_docs_dir):
            ...
            await asyncio.gather(provider.initialize(), provider.initialize(), provider.initialize())
