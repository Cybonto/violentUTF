...
class TestViolentUTFMCPServer:
    ...
    async def test_read_resource_success(self, mcp_server):
        ...
        mock_resource_func = AsyncMock(return_value={
    ...
    async def test_get_prompt_success(self, mcp_server):
        ...
        mock_prompt_func = Mock(return_value={
            "messages": [
    ...
    async def test_run_sse_success(self, mcp_server):
        ...
        with patch.object(mcp_server, 'create_sse_transport', return_value=mock_transport):
...
class TestMCPServerPrompts:
    ...
    def test_red_teaming_prompt(self, mcp_server):
        ...
        assert "messages" in result
        assert any("jailbreak" in msg["content"]["text"].lower() 
...
class TestMCPServerEdgeCases:
    ...
    async def test_concurrent_tool_calls(self, mcp_server):
        ...
        requests = [
            CallToolRequest(name="concurrent_tool", arguments={})
            for _ in range(5)
        ]
        ...
        result_values = [
            json.loads(r.content[0].text)["call"] 
            for r in results
        ]
