...
from app.core.auth import AuthMiddleware, bearer_scheme, api_key_header
...
class TestAuthMiddleware:
    ...
    @patch('time.time')
    def test_is_from_apisix_with_valid_hmac(self, mock_time, auth_middleware, mock_request):
        ...
        with patch.object(auth_middleware, '_verify_apisix_signature', return_value=True):
            result = auth_middleware._is_from_apisix(mock_request)
            assert result is True
    
    @patch('time.time')
    def test_is_from_apisix_expired_timestamp(self, mock_time, auth_middleware, mock_request):
    ...
    def test_verify_apisix_signature_valid(self, mock_settings, auth_middleware, mock_request):
    ...
    @patch('app.core.config.settings')
    def test_verify_apisix_signature_invalid(self, mock_settings, auth_middleware, mock_request):
    ...
    @patch('app.core.config.settings')
    def test_verify_apisix_signature_no_secret(self, mock_settings, auth_middleware, mock_request):
    ...
    async def test_authenticate_jwt_valid(self, mock_decode, auth_middleware, mock_jwt_payload):
    ...
    async def test_authenticate_api_key_valid(self, mock_get_db, auth_middleware, mock_db_session):
    ...
    @pytest.mark.asyncio
    @patch('app.db.database.get_db_session')
    async def test_authenticate_api_key_not_found(self, mock_get_db, auth_middleware, mock_db_session):
    ...
    @pytest.mark.asyncio
    @patch('app.db.database.get_db_session')
    async def test_authenticate_api_key_inactive(self, mock_get_db, auth_middleware, mock_db_session):
    ...
    @pytest.mark.asyncio
    @patch('app.db.database.get_db_session')
    async def test_authenticate_api_key_expired(self, mock_get_db, auth_middleware, mock_db_session):
    ...
    async def test_call_direct_access_blocked(self, mock_log, auth_middleware, mock_credentials):
    ...
    @pytest.mark.asyncio
    async def test_call_jwt_success(self, auth_middleware, mock_request, mock_credentials, mock_jwt_payload):
        """Test successful authentication with JWT"""
        with patch.object(auth_middleware, '_authenticate_jwt') as mock_auth_jwt:
            mock_user = User(username="test-user", email="test@example.com", roles=["user"])
            mock_auth_jwt.return_value = mock_user
            
            user = await auth_middleware(mock_request, credentials=mock_credentials, api_key=None)
            
            assert user == mock_user
            mock_auth_jwt.assert_called_once_with(mock_credentials.credentials)
    
    @pytest.mark.asyncio
    async def test_call_api_key_success(self, auth_middleware, mock_request, mock_api_key):
        ...
        with patch.object(auth_middleware, '_authenticate_api_key') as mock_auth_key:
            ...
            user = await auth_middleware(mock_request, credentials=None, api_key=mock_api_key)
    ...
    @pytest.mark.asyncio
    async def test_call_jwt_takes_precedence(self, auth_middleware, mock_request, mock_credentials, mock_api_key):
        """Test JWT authentication takes precedence over API key"""
        with patch.object(auth_middleware, '_authenticate_jwt') as mock_auth_jwt:
            with patch.object(auth_middleware, '_authenticate_api_key') as mock_auth_key:
                mock_user = User(username="jwt-user", email="jwt@example.com", roles=["user"])
                mock_auth_jwt.return_value = mock_user
...
class TestSecurityScenarios:
    ...
    async def test_sql_injection_in_api_key(self, auth_middleware, mock_request):
        ...
        with patch('app.db.database.get_db_session') as mock_get_db:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await auth_middleware(mock_request, credentials=None, api_key=malicious_key)
    ...
    async def test_jwt_algorithm_confusion(self, auth_middleware, mock_request):
        ...
        with patch('app.core.security.decode_token') as mock_decode:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await auth_middleware(mock_request, credentials=credentials, api_key=None)
    ...
    async def test_timing_attack_prevention(self, auth_middleware):
        ...
        with patch('app.core.config.settings.APISIX_GATEWAY_SECRET', secret):
            auth_middleware._verify_apisix_signature(
                Mock(method="GET", url=Mock(path="/test")),
                correct_sig,
                "123"
            )
