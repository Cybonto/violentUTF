...
class TestJWTManager:
    ...
    def jwt_manager(self):
        ...
        with patch("utils.jwt_manager.JWTManager._load_environment"):
            with patch(
                "utils.jwt_manager.JWTManager._get_api_base_url",
                return_value="http://localhost:9080",
            ):
                manager = JWTManager()
                return manager
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("requests.get") as mock_get:
                ...
                mock_get.assert_called_with(
                    "http://localhost:9080/api/v1/jwt-keys/secret", timeout=10
                )
    ...
    def test_create_token_success(
        self, jwt_manager, valid_token_payload, mock_session_state
    ):
    ...
    def test_create_token_with_expiry(self, jwt_manager, mock_session_state):
        ...
        with patch.object(jwt_manager, "_get_secret_key", return_value="test-secret"):
            token = jwt_manager.create_token(
                payload, expires_delta=timedelta(minutes=15)
            )
    ...
    def test_create_token_error(self, jwt_manager):
        ...
        with patch.object(
            jwt_manager, "_get_secret_key", side_effect=Exception("Secret error")
        ):
    ...
    def test_validate_token_invalid_signature(self, jwt_manager, valid_token_payload):
        ...
        with patch.object(
            jwt_manager, "_get_secret_key", return_value="correct-secret"
        ):
            payload = jwt_manager.validate_token(token)
    ...
    def test_refresh_token_from_keycloak(self, jwt_manager, mock_session_state):
        ...
        with patch.object(
            jwt_manager, "create_token", return_value="new-token"
        ) as mock_create:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("requests.get", side_effect=mock_get):
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                    futures = [
                        executor.submit(jwt_manager._get_secret_key) for _ in range(3)
                    ]
                ...
                assert (
                    call_count <= 2
                )  # May have some race conditions but should be limited
