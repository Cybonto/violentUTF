...
class TestAPISIXModelMapping:
    ...
    def test_openai_model_mapping(self):
        ...
        assert (
            get_apisix_endpoint_for_model("openai", "gpt-3.5-turbo")
            == "/ai/openai/gpt35"
        )
        assert (
            get_apisix_endpoint_for_model("openai", "gpt-4o-mini")
            == "/ai/openai/gpt4o-mini"
        )
        assert (
            get_apisix_endpoint_for_model("openai", "o1-preview")
    ...
    def test_anthropic_model_mapping(self):
        ...
        assert (
            get_apisix_endpoint_for_model("anthropic", "claude-3-opus-20240229")
            == "/ai/anthropic/opus"
        )
        assert (
            get_apisix_endpoint_for_model("anthropic", "claude-3-5-sonnet-20241022")
            == "/ai/anthropic/sonnet35"
        )
        assert (
            get_apisix_endpoint_for_model("anthropic", "claude-opus-4-20250514")
            == "/ai/anthropic/opus4"
        )
    ...
    def test_ollama_model_mapping(self):
        ...
        assert (
            get_apisix_endpoint_for_model("ollama", "mistral") == "/ai/ollama/mistral"
        )

    def test_google_model_mapping(self):
        """Test Google model to endpoint mapping"""
        assert (
            get_apisix_endpoint_for_model("google", "gemini-pro") == "/ai/google/gemini"
        )
        assert (
            get_apisix_endpoint_for_model("google", "gemini-1.5-pro")
            == "/ai/google/gemini15"
        )
...
class TestGeneratorCRUD:
    ...
    async def test_list_generators_with_filters(self, mock_get_db):
        ...
        await list_generators(provider="openai", model="gpt-4", is_active=True)
    ...
    async def test_create_generator_database_error(self, mock_get_db):
        ...
        request = GeneratorCreateRequest(
            name="New Generator", provider="openai", model="gpt-4", parameters={}
        )
...
class TestAPISIXModels:
    ...
    @pytest.mark.asyncio
    async def test_get_apisix_models_success(self):
        ...
        with patch("httpx.get") as mock_get:
            ...
            response = await get_apisix_models()
    ...
    @pytest.mark.asyncio
    async def test_get_apisix_models_error_handling(self):
        """Test error handling in APISIX models retrieval"""
        with patch("httpx.get") as mock_get:
            mock_get.side_effect = Exception("Network error")

            with pytest.raises(HTTPException) as exc_info:
                await get_apisix_models()
