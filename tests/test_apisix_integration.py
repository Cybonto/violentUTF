...
def make_request(
    method: str, url: str, include_auth: bool = True, **kwargs
) -> requests.Response:
...
class TestAPISIXConnectivity:
    ...
    def test_apisix_gateway_running(self):
        ...
        try:
            ...
            assert response.status_code in [
                200,
                404,
            ], f"APISIX should be running, got {response.status_code}"
        except requests.ConnectionError:
            pytest.fail(
                "APISIX Gateway is not running on port 9080. Run: cd apisix && docker compose up -d"
            )
    ...
    def test_health_endpoint_via_apisix(self):
        ...
        if response.status_code == 404:
            pytest.skip(
                "Health endpoint route not configured in APISIX. Run: ./configure_routes.sh"
            )
    ...
    def test_api_docs_via_apisix(self):
        ...
        if response.status_code == 404:
            pytest.skip(
                "Docs endpoint route not configured in APISIX. Run: ./configure_routes.sh"
            )
...
class TestAuthenticationEndpointsViaAPISIX:
    ...
    def test_token_info_endpoint_routing(self):
        ...
        if response.status_code == 404:
            pytest.fail(
                "Authentication route not configured in APISIX. Run: ./configure_routes.sh"
            )
    ...
    def test_token_validate_endpoint_routing(self):
        ...
        response = make_request(
            "POST", APISIX_ENDPOINTS["auth_token_validate"], json=payload
        )
...
class TestDatabaseEndpointsViaAPISIX:
    ...
    def test_database_initialize_routing(self):
        ...
        response = make_request(
            "POST", APISIX_ENDPOINTS["database_initialize"], json=payload
        )

        if response.status_code == 404:
            pytest.fail(
                "Database route not configured in APISIX. Run: ./configure_routes.sh"
            )
    ...
    def test_database_reset_routing(self):
        ...
        response = make_request(
            "POST", APISIX_ENDPOINTS["database_reset"], json=payload
        )
...
class TestSessionEndpointsViaAPISIX:
    ...
    def test_sessions_get_routing(self):
        ...
        if response.status_code == 404:
            pytest.fail(
                "Sessions route not configured in APISIX. Run: ./configure_routes.sh"
            )
    ...
    def test_sessions_schema_routing(self):
        ...
        response = make_request(
            "GET", APISIX_ENDPOINTS["sessions_schema"], include_auth=False
        )
...
class TestConfigEndpointsViaAPISIX:
    ...
    def test_config_parameters_routing(self):
        ...
        if response.status_code == 404:
            pytest.fail(
                "Config route not configured in APISIX. Run: ./configure_routes.sh"
            )
...
class TestFileEndpointsViaAPISIX:
    ...
    def test_files_list_routing(self):
        ...
        if response.status_code == 404:
            pytest.fail(
                "Files route not configured in APISIX. Run: ./configure_routes.sh"
            )
    ...
    def test_files_upload_routing(self):
        ...
        response = requests.post(
            APISIX_ENDPOINTS["files_upload"], headers=headers, timeout=30
        )
...
class TestStartPageEndpoints:
    ...
    def test_all_start_page_endpoints_exist(self):
        ...
        for endpoint_url in found_endpoints:
            ...
            relative_path = (
                endpoint_url.split("localhost:9080")[-1]
                if "localhost:9080" in endpoint_url
                else endpoint_url
            )
        ...
        for endpoint in test_endpoints:
            try:
                response = requests.get(
                    f"{APISIX_BASE_URL}{endpoint}",
                    headers=get_apisix_headers(),
                    timeout=5,
                )
...
class TestJWTAuthenticationIntegration:
    ...
    def test_jwt_token_structure(self):
        ...
        try:
            ...
            assert (
                "ai-api-access" in decoded["roles"]
            ), "JWT missing 'ai-api-access' role"
    ...
    def test_generator_endpoints_routing(self):
        ...
        for endpoint in generator_endpoints:
            ...
            assert (
                response.status_code != 404
            ), f"Generator endpoint {endpoint} not routed through APISIX"
    ...
    def test_apisix_api_key_endpoints(self):
        ...
        assert (
            "apikey" in api_key_headers
        ), "APISIX API key header should use 'apikey' field"
        assert (
            api_key_headers["X-API-Gateway"] == "APISIX"
        ), "Should include APISIX gateway marker"
...
class TestAuthenticationErrorHandling:
    ...
    def test_missing_jwt_token(self):
        ...
        response = make_request(
            "GET",
            f"{APISIX_BASE_URL}/api/v1/auth/token/info",
            include_auth=False,
    ...
    def test_authentication_error_messages(self):
        ...
        response = make_request(
            "GET", f"{APISIX_BASE_URL}/api/v1/auth/token/info", include_auth=False
        )
