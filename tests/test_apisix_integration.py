...
class TestFileEndpointsViaAPISIX:
    ...
    def test_files_upload_routing(self):
        ...
        assert response.status_code in [400, 401, 403, 422], f"Unexpected status: {response.status_code}: {response.text}"
...
class TestStartPageEndpoints:
    ...
    def test_all_start_page_endpoints_exist(self):
        ...
        for endpoint_url in found_endpoints:
            ...
            relative_path = endpoint_url.split('localhost:9080')[-1] if 'localhost:9080' in endpoint_url else endpoint_url
            ...
            try:
                ...
                assert response.status_code != 404, f"Route not configured for {relative_path}. Run: ./configure_routes.sh"
...
class TestDebugHelpers:
    ...
    def test_print_apisix_status(self):
        ...
        for endpoint in test_endpoints:
            try:
                response = requests.get(f"{APISIX_BASE_URL}{endpoint}", 
...
class TestAuthenticationErrorHandling:
    ...
    def test_missing_jwt_token(self):
        ...
        response = make_request("GET", f"{APISIX_BASE_URL}/api/v1/auth/token/info", 
                               include_auth=False, headers=headers_without_auth)
    ...
    def test_malformed_jwt_token(self):
        ...
        response = requests.get(f"{APISIX_BASE_URL}/api/v1/auth/token/info", 
                               headers=malformed_headers, timeout=10)
