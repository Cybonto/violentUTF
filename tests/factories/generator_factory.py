...
import factory
from factory import DictFactory, Faker, LazyAttribute, SubFactory
...
class GeneratorFactory(factory.Factory):
    ...
    model = factory.LazyAttribute(
        lambda obj: {
            ...
            "huggingface": factory.Iterator(
                ["meta-llama/Llama-2-7b", "mistralai/Mistral-7B"]
            ),
...
class OpenAIGeneratorFactory(GeneratorFactory):
    ...
    def parameters(self):
        ...
        params.update({"model": self.model, "stream": False, "n": 1, "logprobs": False})
        return params
...
def create_diverse_generators() -> list[Dict[str, Any]]:
    ...
    return [
        ...
        create_test_generator(
            provider="google", model="gemini-pro", name="Google Gemini"
        ),
