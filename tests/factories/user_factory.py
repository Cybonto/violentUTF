...
class UserFactory(factory.Factory):
    ...
    last_login = factory.LazyAttribute(
        lambda obj: obj.created_at + timedelta(days=factory.Faker('pyint', min_value=1, max_value=30).generate())
    )
    ...
    permissions = factory.LazyAttribute(lambda obj: {
        ...
        'developer': ['read:all', 'write:generators', 'write:datasets', 'execute:all']
...
class APIUserFactory(UserFactory):
    ...
    roles = ['api_user']
    is_api_user = True
    api_key = factory.LazyFunction(lambda: f"vutf_{''.join(factory.Faker('lexify', text='????????').generate() for _ in range(4))}")
...
class JWTTokenFactory(factory.Factory):
    ...
    sub = factory.LazyAttribute(lambda obj: obj.user.get('id') if hasattr(obj, 'user') else str(uuid.uuid4()))
    email = factory.LazyAttribute(lambda obj: obj.user.get('email') if hasattr(obj, 'user') else Faker('email').generate())
    name = factory.LazyAttribute(lambda obj: obj.user.get('name') if hasattr(obj, 'user') else Faker('name').generate())
    preferred_username = factory.LazyAttribute(lambda obj: obj.user.get('preferred_username') if hasattr(obj, 'user') else Faker('user_name').generate())
    ...
    # Roles and permissions
    roles = factory.LazyAttribute(lambda obj: obj.user.get('roles', ['user']) if hasattr(obj, 'user') else ['user'])
...
def create_keycloak_token_response(user: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
...
def create_authenticated_request_headers(user: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    """
