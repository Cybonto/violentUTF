...
class PromptFactory(factory.Factory):
    ...
    tags = factory.LazyAttribute(
        lambda obj: [
            obj.category,
            obj.severity,
            f"test_{factory.Faker('word').generate()}",
        ]
    )
...
class DatasetFactory(factory.Factory):
    ...
    def statistics(self):
        ...
        return {
            ...
            "avg_prompt_length": (
                sum(len(p["text"].split()) for p in self.prompts) // len(self.prompts)
                if self.prompts
                else 0
            ),
        }
    ...
    # Additional fields
    tags = factory.List([factory.Faker("word"), factory.Faker("word"), "test"])
        ...
        for category in categories:
            for severity in ["low", "medium", "high"]:
                for _ in range(3):  # 3 prompts per category/severity combo
                    prompt = PromptFactory(category=category, severity=severity)
                    prompts.append(prompt)
...
def create_diverse_datasets() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        create_test_dataset(name="Small Test Set", prompts=create_prompt_batch(5)),
        create_test_dataset(name="Large Dataset", prompts=create_prompt_batch(100)),
        create_csv_dataset(name="CSV Export Test"),
        create_test_dataset(
            name="Private Dataset", is_public=False, tags=["private", "internal"]
        ),
        create_test_dataset(name="Inactive Dataset", is_active=False),
    ]
    ...
    pyrit_prompts = [
        {
            ...
            "category": category,
            "severity": "high",
            "metadata": {"scorer": scorer, "converter": converter},
        }
