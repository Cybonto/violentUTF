...
def mock_auth():
    ...
    with patch(
        "violentutf_api.fastapi_app.app.core.auth.get_current_user"
    ) as mock_auth:
...
def test_execute_orchestrator(mock_auth, mock_db):
    ...
    with patch(
        ...
        response = client.post(
            "/api/v1/orchestrators/test-uuid/execute", json=execution_data
        )
...
def test_execute_orchestrator_not_found(mock_auth, mock_db):
    ...
    response = client.post(
        "/api/v1/orchestrators/nonexistent-uuid/execute", json=execution_data
    )
