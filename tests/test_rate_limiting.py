...
TEST_ENDPOINTS = {
    ...
    "/auth/token/info": {
        "method": "GET",
        "expected_limit": 30,  # 30 per minute
        "requires_auth": True,
    },
}
...
async def test_rate_limiting():
    ...
    async with aiohttp.ClientSession() as session:
        for endpoint, config in TEST_ENDPOINTS.items():
            ...
            errors = [
                r for r in responses if r["status"] not in [200, 201, 401, 403, 429]
            ]
            ...
            if len(rate_limited) > 0:
                print(
                    f"   ✅ PASS: Rate limiting is working! Blocked {len(rate_limited)} requests"
                )
    ...
    for endpoint, result in results.items():
        ...
        print(
            f"{status} {endpoint}: {result['rate_limited']}/{result['total_requests']} requests rate limited"
        )
