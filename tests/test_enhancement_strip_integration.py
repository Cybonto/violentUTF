    ...
    def test_analysis_uses_real_mcp_tools(self, mcp_client, test_prompt):
        ...
        try:
            security_result = mcp_client.execute_tool(
                "analyze_security", {"text": test_prompt}
            )
            if security_result:
    ...
    def test_test_variations_generated_by_mcp(self, mcp_client, test_prompt):
        ...
        try:
            jailbreak_var = mcp_client.get_prompt(
                "jailbreak_test",
                {"scenario": "test scenario", "target_query": test_prompt},
            )
            if jailbreak_var:
                variations_found.append(
                    {
                        "type": "jailbreak",
                        "content": jailbreak_var,
                        "source": "mcp_prompt",
                    }
                )
        except Exception as e:
        ...
        try:
            bias_var = mcp_client.get_prompt(
                "bias_test", {"context": test_prompt, "topic": "programming"}
            )
            if bias_var:
                variations_found.append(
                    {"type": "bias", "content": bias_var, "source": "mcp_prompt"}
                )
        except Exception as e:
        ...
        test_tools = [
            ...
            "generate_privacy_test",
            "generate_harmful_test",
        ]
    ...
    def test_natural_language_parser_integration(self, mcp_client):
        ...
        for cmd in test_commands:
            ...
            parsed_commands.append(
                {"input": cmd, "type": result.type.value, "args": result.arguments}
            )
            print(
                f"Parsed '{cmd}' -> Type: {result.type.value}, Args: {result.arguments}"
            )
                ...
                if "error" not in config:
                    ...
                    if "error" not in result:
                        successful_tests.append(
                            {"type": test_type, "config": config, "result": result}
                        )
    ...
    def test_enhancement_strip_workflow(self, mcp_client, test_prompt):
        ...
        try:
            ...
            for prompt_name in ["jailbreak_test", "bias_test", "security_test"]:
                try:
                    var = mcp_client.get_prompt(
                        prompt_name,
                        {"target_query": test_prompt, "context": test_prompt},
                    )
                    if var:
    ...
    def test_performance_with_real_operations(self, mcp_client, test_prompt):
        ...
        try:
            ...
            performance_results.append(
                ("enhancement", enhancement_time, enhanced is not None)
            )
