...
try:
    ...
    ChatModel = None
    logger.warning(
        "Google Vertex AI libraries not installed. Vertex AI features will be disabled."
    )
        ...
        if not project_id or not location or not service_account_info:
            ...
            if model_names:
                ...
                if selected_model:
                    # Test the model by making a small query
                    try:
                        if VERTEXAI_AVAILABLE and vertexai:
                            vertexai.init(
                                project=project_id,
                                location=location,
                            ...
                            st.error(
                                "Vertex AI libraries are not installed. Please install google-cloud-aiplatform."
                            )
                    except Exception as e:
                        st.error(f"Failed to access the model: {e}")
                        st.stop()
                ...
                with st.spinner("Generating response..."):
                    try:
                        if VERTEXAI_AVAILABLE and vertexai:
                            ...
                            raise Exception("Vertex AI libraries are not installed")
