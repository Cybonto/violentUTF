#!/bin/bash
  ...
  if ! curl -s --connect-timeout 5 --max-time 10 "${base_url%/}${spec_path}" > /dev/null 2>&1;
  then
          ...
          if isinstance(path_item, dict) and '$ref' not in path_item:
              for method in ['get', 'post', 'put', 'delete', 'patch', 'head', 'options']:
                  if method in path_item:
                      ...
                      op_id = operation.get('operationId', f"{method}_{path.replace('/', 
  '_').replace('{', '').replace('}', '')}")
                      ...
                      is_ai_endpoint = any(keyword in path.lower() or keyword in
  endpoint['summary'].lower()
                                         for keyword in ['chat', 'completion', 'generate',
  'predict', 'inference', 'embedding'])
  ...
  try:
      ...
      for ep in endpoints:
          print(f\"{ep['path']}|{ep['method']}|{ep['operationId']}|{ep.get('is_ai_endpoint', 
  False)}\")
  except Exception as e:
  ...
  if [ -f "$temp_endpoints" ] && [ -s "$temp_endpoints" ]; then
      ...
      while IFS='|' read -r path method op_id is_ai; do
          ...
          safe_operation_id=$(echo "$op_id" | sed 's/[^a-zA-Z0-9-]/-/g' | tr '[:upper:]'
  '[:lower:]')
